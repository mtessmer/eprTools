...
class CW_spec:
    ...
    def basecorr(self, k = 0, ends = 100):
        ...
        jSpec = cumtrapz(self.spec, self.field, initial = 0)
        ...
        plt.title('this')
        plt.plot(self.field, iSpec)
        plt.scatter(self.field[myList], iSpec[myList])
        plt.plot(self.field, jSpec)
        plt.show()
    ...
    def center(self, center_field = 0):
        ...
        midPoint = self.field[Xmax + subMidpoint]
        
        #Set field s.t. the spectral midpoint is at center_field
        self.field = self.field - midPoint + center_field
